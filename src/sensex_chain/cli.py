...
from .future_depth import FyersFutureDepthEnricher
...
def main(argv:list[str]|None=None)->int:
 ...
 try:
  ...
  worker=LiveChainWorker(catalog,FallbackTokenProvider(automated,fallback),lambda token:FyersDataFeed(f'{config.fyers_client_id}:{token}'),cache,gateway,SystemClock(),config.flush_seconds,option_chain_factory=lambda token:FyersOptionChainEnricher(config.fyers_client_id,token),future_depth_factory=lambda token:FyersFutureDepthEnricher(config.fyers_client_id,token))
